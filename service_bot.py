...
PATH_UNDEFINED_MODEL_NAME_LIST_LOCK = PATH + "data/undefined_model_name.lock"
PATH_MODEL_NAME_MSG_IN_TELEGRAM = 'data/msg_in_telegram.dat'
...
def save_approved_model_name(model_name):
    print('Сохраняю в список разрешенных имен: "{}"'.format(model_name))
...
def save_changed_model_name(prev_model_name, new_model_name):
    print('Сохраняю в словарь БД: [{}] -> [{}]'.format(prev_model_name.lower(), new_model_name.lower()))
...
def save_true_letter_case_from_telegram(prev_model_name, new_model_name):
    for item in BRAND_NAME_LIST:
        ...
        if item in prev_model_name.lower():
            ...
            print('Сохраняю в словарь ТЕЛЕГРАМ: [{}] -> [{}]'.format(prev_model_name, new_model_name))
            break
...
def command_man(message):
    ...
    bot.send_message(message.chat.id, TEXT_ADD_MANUALLY.format(left, right), parse_mode='html')
...
def approve_no_callback(query):
    ...
    bot.edit_message_text(TEXT_ENTER_TRUE_NAME.format(model_name), query.message.chat.id, query.message.id, parse_mode='html')
    bot.register_next_step_handler(query.message, no_answer_get_true_name, (query.message.id, model_name))
    is_waiting_approve = False
...
def check_new_data_in_file():
    while True:
        ...
        if os.path.isfile(PATH_UNDEFINED_MODEL_NAME_LIST_LOCK):
            time.sleep(3)
            continue
        ...
        if models_list:
            print("Есть данные: {} шт".format(len(models_list)))
            ...
            print("После фильтра осталось {}".format(len(models_list)))
...
if __name__ == '__main__':
    print("Start")
    check_data_thread = threading.Thread(target=check_new_data_in_file, daemon=True)
