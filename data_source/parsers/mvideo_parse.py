...
from data_source.parsers.parse_base import ParseBase
...
class MVideoParse(ParseBase):
    ...
    def __init__(self):
        super().__init__(domain="https://www.mvideo.ru", shop="mvideo", logger=logger)

    def _wd_city_selection_catalog(self):
        """
        Алгоритм выбора города для всех возможных ситуаций для страницы каталога
        """
        city = self._wd_find_elem_with_timeout(By.XPATH, "//span[@class='location-text top-navbar-link']")
        if not city:
    ...
    def _wd_next_page(self):
        ...
        for num_try in range(3):
            ...
            try:
                ...
                self.logger.error('По непонятной причине список pr_result_list[-5] оказался пуст, выход за границы списка')
                return False
